...
def run(
    ...
    for batch_i, (im, targets, paths, shapes, masks) in enumerate(pbar):
        ...
        with dt[1]:
            ...
            if compute_loss:
                ...
                loss = compute_loss((dec_bboxes, dec_scores), _targets, masks=masks, protos=protos)
                loss, loss_items = sum(loss.values()), torch.as_tensor(
                    [loss[k].detach() for k in ["loss_giou", "loss_class", "loss_bbox", "loss_mask"]], device=device
                )
        ...
        for si, (bbox, proto) in enumerate(zip(bboxes, protos)):
    ...
    LOGGER.info('%22s' + '%11.3g' * 4 % ('val/loss', *(mloss.cpu() / len(dataloader)).tolist()))
    if nt.sum() == 0:
