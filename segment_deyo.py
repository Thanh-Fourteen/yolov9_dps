...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    compute_loss = RTDETRSegmentLoss(nc, use_vfl=True)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%11s' * 9) %
                    ('Epoch', 'GPU_mem', "loss_giou", "loss_class", "loss_bbox", "loss_mask", "loss_dice", 'Instances', 'Size'))
        if RANK in {-1, 0}:
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch ------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                dec_bboxes, dec_scores, dec_masks, enc_bboxes, enc_scores, enc_masks, dn_meta = pred
                if dn_meta is None:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default= 'train_seg', help='save to project/name')
    parser.add_argument('--name', default='gelan-c-seg', help='save to project/name')
